...
class RunningMeanStd_2:
    # Dynamically calculate mean and std
    def __init__(self, shape):  # shape:the dimension of input data
        self.n = 0
        self.mean = torch.zeros(shape, 1)
        self.S = torch.zeros(shape, 1)
...
class Normalization:
    def __init__(self, shape):
        self.running_ms = RunningMeanStd_2(shape=shape)
...
class RewardScaling:
    def __init__(self, shape, gamma):
        ...
        self.running_ms = RunningMeanStd_2(shape=self.shape)
        self.R = torch.zeros((self.shape, 1))
