...
class EpsilonGreedy:
    ...
    def choose_actions(self, q_vals: dict, t: int, rng: chex.PRNGKey):
        def explore(q, eps, key):
            ...
            random_actions = jax.random.randint(key_a, shape=greedy_actions.shape, minval=0,
                                                maxval=q.shape[-1])  # sample random actions
...
def make_train(config):
    ...
    def train(rng):
        ...
        test_env = CTRolloutManager(env, batch_size=config[
            "NUM_TEST_EPISODES"])  # batched env for testing (has different batch size)
        ...
        if config.get('PARAMETERS_SHARING', True):
            ...
            init_hs = ScannedRNN.initialize_carry(config['AGENT_HIDDEN_DIM'], len(env.agents),
                                                  1)  # (n_agents, batch_size, hidden_dim)
        ...
        if config.get('PARAMETERS_SHARING', True):
            def homogeneous_pass(params, hidden_state, obs, dones):
                ...
                batched_input = (
                    ...
                    jnp.concatenate([dones[agent] for agent in agents], axis=1),
                # ensure to not pass other keys (like __all__)
                )
                hidden_state, q_vals = agent.apply(params, hidden_state, batched_input)
                q_vals = q_vals.reshape(original_shape[0], len(agents), *original_shape[1:-1],
                                        -1)  # (time_steps, n_agents, n_envs, action_dim)
            ...
            def homogeneous_pass(params, hidden_state, obs, dones):
                ...
                batched_input = (
                    ...
                    jnp.stack([dones[agent] for agent in agents], axis=0),
                # ensure to not pass other keys (like __all__)
                )
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(step_state, unused):
                ...
                obs_ = {a: last_obs[a] for a in
                        env.agents}  # ensure to not pass the global state (obs["__all__"]) to the network
                ...
                valid_q_vals = jax.tree_util.tree_map(lambda q, valid_idx: q.squeeze(0)[..., valid_idx], q_vals,
            ...
            rng, _rng = jax.random.split(rng)
            if config.get('PARAMETERS_SHARING', True):
                hstate = ScannedRNN.initialize_carry(config['AGENT_HIDDEN_DIM'],
                                                     len(env.agents) * config["NUM_ENVS"])  # (n_agents*n_envs, hs_size)
            else:
                hstate = ScannedRNN.initialize_carry(config['AGENT_HIDDEN_DIM'], len(env.agents),
                                                     config["NUM_ENVS"])  # (n_agents, n_envs, hs_size)
            ...
            def _loss_fn(params, target_agent_params, init_hs, learn_traj):
                ...
                obs_ = {a: learn_traj.obs[a] for a in
                        env.agents}  # ensure to not pass the global state (obs["__all__"]) to the network
                ...
                valid_q_vals = jax.tree_util.tree_map(lambda q, valid_idx: q[..., valid_idx], q_vals,
                                                      wrapped_env.valid_actions)
                ...
                targets = jax.tree_map(
                    compute_target,
                    target_max_qvals,
                    {agent: learn_traj.rewards[agent] for agent in env.agents},
                    # rewards and agents could contain additional keys
            ...
            if config.get('PARAMETERS_SHARING', True):
                init_hs = ScannedRNN.initialize_carry(config['AGENT_HIDDEN_DIM'], len(env.agents) * config[
                    "BUFFER_BATCH_SIZE"])  # (n_agents*batch_size, hs_size)
                ...
                def _loss_fn(params, target_params, init_hs, obs, dones, actions, valid_actions, rewards):
                    ...
                    valid_actions = valid_actions.reshape(*[1] * len(q_vals.shape[:-1]),
                                                          -1)  # reshape to match q_vals shape
                    valid_argmax = jnp.argmax(
                        jnp.where(valid_actions.astype(bool), jax.lax.stop_gradient(q_vals), -1000000.), axis=-1)
                    target_max_qvals = q_of_action(target_q_vals, valid_argmax)[
                                       1:]  # target q_vals of greedy actions, avoiding first time step
                    targets = compute_target(target_max_qvals, rewards, dones)
                    return jnp.mean((chosen_action_qvals - jax.lax.stop_gradient(targets)) ** 2)

                init_hs = ScannedRNN.initialize_carry(config['AGENT_HIDDEN_DIM'], len(env.agents),
                                                      config["BUFFER_BATCH_SIZE"])  # (n_agents, batch_size, hs_size)
                grad_fn = jax.value_and_grad(_loss_fn, has_aux=False)
        ...
        def get_greedy_metrics(rng, params, time_state):
            ...
            def _greedy_env_step(step_state, unused):
                ...
                dones_ = jax.tree_map(lambda x: x[np.newaxis, :], last_dones)
                hstate, q_vals = homogeneous_pass(params, hstate, obs_, dones_)
                actions = jax.tree_util.tree_map(lambda q, valid_idx: jnp.argmax(q.squeeze(0)[..., valid_idx], axis=-1),
                                                 q_vals, test_env.valid_actions)
            ...
            init_dones = {agent: jnp.zeros((config["NUM_TEST_EPISODES"]), dtype=bool) for agent in
                          env.agents + ['__all__']}
            rng, _rng = jax.random.split(rng)
            if config["PARAMETERS_SHARING"]:
                hstate = ScannedRNN.initialize_carry(config['AGENT_HIDDEN_DIM'], len(env.agents) * config[
                    "NUM_TEST_EPISODES"])  # (n_agents*n_envs, hs_size)
            else:
                hstate = ScannedRNN.initialize_carry(config['AGENT_HIDDEN_DIM'], len(env.agents),
                                                     config["NUM_TEST_EPISODES"])  # (n_agents, n_envs, hs_size)
            step_state = (
                params,
                env_state,
                init_obs,
            ...
            first_infos = jax.tree_map(lambda i: jax.vmap(first_episode_returns, in_axes=1)(i[..., 0], all_dones),
                                       infos)
            metrics = {
            ...
            if config.get('VERBOSE', False):
                ...
                jax.debug.callback(callback, time_state['timesteps'] * config['NUM_ENVS'],
