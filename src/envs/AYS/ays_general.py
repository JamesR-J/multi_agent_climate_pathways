...
def create_figure(*bla, S_scale=1e9, W_scale=1e12, W_mid=None, S_mid=None, boundaries=None,
                  transformed_formatters=False,
    ...
    if "A_max" in kwargs:
        ...
        raise KeyError(
            "can't find proper key for 'A' in kwargs that determines which representation of 'A' has been used")
...
def load_result_file(fname, *,
    ...
    if auto_reformat:
        header, data = _reformat(header, data, verbose=verbose)
        return header, data
    raise IOError("please reformat the file (from version {} to {})".format(
        versioninfo2version(header.pop("aws-version-info", DEFAULT_VERSION_INFO)), __version__))
    ...
    for sig in ALL_SIGNALS:
        ...
        if set([sig, sigclass, signum]).intersection(sigs):
            try:
                ...
                if verbose:
                    print("ignoring signal registration: [{:>2d}] {} (because {}: {!s})".format(ALL_SIGNALS[sig], sig,
